...
(() => {
  ...
  function createIconComponent(iconClone, size, strokeWeight, originalWidth, originalHeight) {
    ...
    iconClone.resize(scaledSize83Width, scaledSize83Height);
  ...
  function styleComponentSet(componentSet) {
    try {
  ...
  function setComponentConstraints(componentSet) {
  ...
  function getVectorsFromGroup(group) {
  ...
  function validateAndExtractWorkingFrame(sourceNode) {
  ...
  var COMPONENT_SET_WIDTH = 464;
  var COMPONENT_SET_HEIGHT = 80;
...
`;
  ...
  async function createIconSet(sourceNode, iconName, yPosition, xPosition) {
  ...
  var ICON_SIZE = 24;
  var STROKE_WEIGHT = 2;
  async function processBatchSingle(selectedNodes) {
    const invalidFrames = [];
    for (const node of selectedNodes) {
      const nodeName = node.name || "Unnamed";
      if (!isValidName(nodeName)) {
...
`;
  ...
  async function createIconSingle(sourceNode, iconName, yPosition, xPosition) {
    let workingFrame = null;
    try {
      const extraction = validateAndExtractWorkingFrame(sourceNode);
      workingFrame = extraction.workingFrame;
...
<body>
  ...
  <div class="content-wrapper">
    ...
    <div class="feedback-view" id="feedbackView">
      <div class="builder-header">
        <button class="back-button" id="feedbackBackBtn">
        ...
        <h2 class="builder-title">Feedback</h2>
      </div>
          ...
          <div class="form-group">
            ...
            <div class="custom-dropdown" id="feedbackDropdown">
              <button type="button" class="dropdown-trigger">
                <span id="dropdownValue" class="placeholder">Select a feature...</span>
              ...
              <input type="hidden" id="feedbackFeature" name="feedbackFeature" value="" required />
            </div>
          </div>
      ...
      <div class="button-section">
        <button type="submit" class="create-btn" form="feedbackForm">Send Feedback</button>
      </div>
    </div>
  ...
  <script>
    ...
    window.onmessage = (event) => {
      ...
      if (msg && msg.type === 'feedback-success') {
        showFeedbackMessage(msg.message, 'success');
        document.getElementById('feedbackForm').reset();
        document.getElementById('fileList').innerHTML = '';
        selectedFiles = [];
      }

      if (msg && msg.type === 'feedback-error') {
        showFeedbackMessage(msg.message, 'error');
      }
    };
    ...
    // ===== Feedback Form =====
    const feedbackLinkBtn = document.getElementById('feedbackLinkBtn');
    ...
    feedbackFiles.addEventListener('change', (e) => {
      ...
      feedbackFiles.value = '';  // Reset so same file can be uploaded again
      updateFileList();
    });
        ...
        item.innerHTML = \`
          ...
          <button type="button" onclick="removeFile(\${index})">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none">
              <path d="M3 6h18M8 6V4a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v2m-1 0v14a1 1 0 0 1-1 1H9a1 1 0 0 1-1-1V6h12zM10 10v8M14 10v8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </button>
        \`;
    ...
    feedbackForm.addEventListener('submit', async (e) => {
      ...
      const feature = feedbackFeature.value;
